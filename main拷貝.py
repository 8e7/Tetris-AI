...
'''
Extra Parameters
-Slant Factor: How much the AI prefers stacking on the left side
-Stack Height: A free zone where stacking below it isn't penalized
-Potential lines: How many lines can be obtained in one line bar
-Different play style based on different heights
'''
'''
pop = []
size = 50
for i in range(size):
    pop.append(tuple(random.random() for i in range(8)))
...
for gen in range(50):
    ...
    while num < size - 1:
        ...
        for i in range(8):
            ...
            if random.random() < 0.07122: #mutation
                if random.randint(0, 1) == 0:
                    ch1[i] += random.uniform(-0.5/(gen + 1), 0.5/(gen + 1))
                else:
                    ch2[i] += random.uniform(-0.5/(gen + 1), 0.5/(gen + 1))
        pop.append(ch1)
        num += 1
        if num < size - 1:
            pop.append(ch2)
